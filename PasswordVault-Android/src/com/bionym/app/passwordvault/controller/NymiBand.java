...
public class NymiBand extends Nymi{
	protected static final String LOG_TAG = NymiBand.class.getName();
	...
	protected static NymiBand instance;
	 ...
	 */
	public static synchronized NymiBand startNclSession(Context context, String name, NclCallback sessionCallback) {
		clearConnectedHandle(false);
		if (instance == null) { // Create the NclCallback object
			NymiBand nymi = new NymiBand();
			nymi.startSession(context, name, sessionCallback);
			instance = nymi;
		} else if (instance.getClass().equals(NymiBand.class)) {
	 ...
	 */
	public static NymiBand getInstance() {
		return instance;
	}
	...
	protected void startSession(Context context, String name, NclCallback sessionCallback) {
		...
		if (bluetoothManager == null) {
			...
			if (bluetoothManager == null) {
				Log.e(LOG_TAG, "Cannot get BluetoothManager!");
				throw new NymiInitializationException("BLE manager initialization failed!");
			}
		}
		...
		if (adapter == null) {
			Log.e(LOG_TAG, "Cannot get BluetoothAdapter!");
			throw new NymiInitializationException("BLE adapter initialization failed!");
		}

		// Initialize NCL
		Ncl.setAdapter(adapter);

		Ncl.setIpAndPort(Constants.IP, 9089);

		if (Ncl.init(nclCallbacks, null, name, NclMode.NCL_MODE_DEFAULT, context)) {
			originalBleStatus = getBleStatus();
		} else { // failed? cleanup and try again
	...
	/**
	 * 
	 * @return BLE_STATUS_OK if BLE is enabled,
	 *         BLE_STATUS_DISABLED_AIRPLANE_MODE if the device is in airplane
	 *         mode, BLE_STATUS_DISABLED is the BLE is disabled,
	 *         BLE_STATUS_NO_BLE is the device has no BLE
	 */

	public int getBleStatus() {
		if (adapter != null) {
			if (isAirplaneModeOn()) {
				return BLE_STATUS_DISABLED_AIRPLANE_MODE;
			}
			if (adapter.isEnabled()) {
				return BLE_STATUS_OK;
			} else {
				return BLE_STATUS_DISABLED;
			}
		}
		return BLE_STATUS_NO_BLE;
	}
	...
	public int enableBle() {
		int bleStatus = BLE_STATUS_OK;
		synchronized (this) {
			// wait for a max of 3 seconds for the BLE
			// adapter to enable
			bleStatus = getBleStatus();
			;
			if (bleStatus == BLE_STATUS_DISABLED) {
				Log.d(LOG_TAG, "BLE disabled, enable it ...");
				if (!adapter.isEnabled()) {
					adapter.enable();
					long time = 0;
					do {
					...
					} while (!adapter.isEnabled() && time < 3000);

					try { // wait for the BLE service to start
						wait(500);
					} catch (InterruptedException e) {
					}
				}
	...
	public int disableBle() {
		...
		synchronized (this) {
			...
			bleStatus = getBleStatus();
			;
			if (errorCount > 0 && bleStatus == BLE_STATUS_OK) { // can disable
																// if we have
																// errors
				Log.d(LOG_TAG, "BLE enabled, disable it ...");
				if (adapter.isEnabled()) {
					adapter.disable();
					long time = 0;
					do {
